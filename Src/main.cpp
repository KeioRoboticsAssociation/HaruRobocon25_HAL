...
UartLinkSubscriber<bool> sub_TapeLED(uart_link, 3);
UartLinkSubscriber<bool> sub_ball2(uart_link, 4);
...
void servoCallback_kokuban(bool servo_state) {
    ...
    if (servo_state == true) 
    {
        ...
        //　上げる
        // 120度から0度まで3度ずつ減らす
        for(int angle = 219; angle >= 58; angle -= 3){
            servoMotor1.SetAngle(angle);
            HAL_Delay(15);
        }
        HAL_Delay(1000);
        ...
        //　もとの位置に戻る
        // 0度から30度まで3度ずつ増やす
        for(int angle = 58; angle <= 90; angle += 3){
            servoMotor1.SetAngle(angle);
            HAL_Delay(10);
        }
    }
...
//         //　上げる
//         // 90度から0度まで3度ずつ減らす
//         for(int angle = 195; angle >= 135; angle -= 3){
//             servoMotor1.SetAngle(angle);
//             HAL_Delay(15);
//         }
//     }
...
void servoCallback_ball2(bool servo_state) {
    // ここにコールバック関数の処理を書く
    if (servo_state == true)
    {
        for(int angle = 135; angle >= 66; angle -= 3){
            servoMotor1.SetAngle(angle);
            HAL_Delay(15);
        }
        HAL_Delay(400);
...
void indicate(bool Tape_LED){
    // ここにコールバック関数の処理を書く
    if (Tape_LED == 1)
    {
        led_tape.clear();
        for (int d=0; d<=20; d++){
            led_tape.set_rgb(d,180,0,0);
        }
        led_tape.show();
        HAL_Delay(1000);
        led_tape.clear();
        for (int d=0; d<=20; d++){
            led_tape.set_rgb(d,0,180,0);
        }
        led_tape.show();
        HAL_Delay(1000);
        led_tape.clear();
        for (int d=0; d<=20; d++){
            led_tape.set_rgb(d,0,0,180);
        }
        led_tape.show();
        HAL_Delay(1000);
        led_tape.clear();
...
void setup() {
    ...
    sub_TapeLED.set_callback(indicate);
    sub_ball2.set_callback(servoCallback_ball2);
